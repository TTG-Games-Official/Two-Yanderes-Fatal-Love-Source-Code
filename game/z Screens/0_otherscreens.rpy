...
screen countdown():
    ...
    hbox:
        ...
        timer 0.1 action If(time > 1, SetVariable("time", time - 0.1), [Hide("countdown"), SetVariable("time", 0), Jump(timeout_label)]) repeat True
        
        bar:
            value AnimatedValue(value=time - 1, range=timer_range - 1, delay=0.1)
            xmaximum 300
...
screen rickroll():
    ...
    if not "rickrolled" in persistent.unlocked_achievement_ids:   
        ...
        add localized_image("ac_rickrolled") at show_slide_achievement
            
        if "loyal_player" in persistent.unlocked_achievement_ids:
            add localized_image("ac_loyal_player") at show_slide_achievement2

        timer 8.0 action Hide("rickroll")
